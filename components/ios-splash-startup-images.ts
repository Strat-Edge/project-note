// Splash screen PWA iOS — mécanisme non-standard mais toujours fonctionnel (Web App Manifest
// n'a pas de champ "splash image", cf. Dev Notes Story 1.3). Consommé par
// metadata.appleWebApp.startupImage dans app/layout.tsx (API Metadata native de Next.js —
// génère les balises <link rel="apple-touch-startup-image"> automatiquement, cf.
// node_modules/next/dist/docs/01-app/03-api-reference/04-functions/generate-metadata.md#appleWebApp).
// Un lien par taille/orientation d'appareil, généré depuis
// Strat'Edge/Branding/Logos/Strat’Edge_h_b_slogan.png (fond colors.header-bg #0F2A44) via
// pwa-asset-generator — regénérer avec scripts/generate-icons.mjs (icônes) + la commande
// pwa-asset-generator (splash, cf. Dev Notes Story 1.3) si le logo change.
export const iosSplashStartupImages: { url: string; media: string }[] = [
