...
- [x] [Review][Decision] Visibilité des tâches "Sans projet" quand un filtre de projet actif est sélectionné — **résolu par Guillaume (2026-08-19) : comportement actuel confirmé.** Les tâches "Sans projet" restent masquées dès qu'un filtre de projet actif est sélectionné, cohérent avec le texte littéral de l'AC#1. Aucun changement de code nécessaire. — ⚠️ **Décision révisée le 2026-09-21, cf. l'entrée Post-livraison ci-dessous.**
- [x] [Post-livraison][Decision] **Révision de la décision du 2026-08-19 (2026-09-21)** — les tâches générales ne sont plus masquées par une sélection de projets. Retour de Guillaume en usage réel : *"lorsqu'on crée une tâche qui n'est pas liée à un projet, ça apparaît dans le calendrier mais comment je trouve la tâche ? je ne peux pas retourner dessus vu que je n'ai accès qu'à des dashboard de projet"*. La lecture littérale de l'AC#1 n'était pas fausse en soi, mais elle s'ajoutait à deux autres trous qui rendaient la tâche générale introuvable : aucun détail de tâche n'existait hors d'une vue projet (`TaskDetail` est interne à `project-view.tsx`), et une tâche générale **sans** échéance n'apparaissait sur aucun écran — y compris celles produites par une suppression définitive de projet, qui reverse les tâches en `projectId: null`. `filterTasksForCalendar` accepte désormais l'identifiant réservé `NO_PROJECT_FILTER_ID` dans `selectedProjectIds` : « Hors projet » se coche comme un projet réel, et l'entrée virtuelle du même nom donne enfin une destination à ces tâches [domain/calendar.ts, app/general-screen.tsx, app/projects/hors-projet/page.tsx, app/projects/projects-screen.tsx, app/projects/[id]/project-view.tsx, data/local/tasks.ts]. Réajustement explicitement prévu par les Dev Notes ci-dessous. Specs mises à jour en conséquence : FR-2 (entrée « Hors projet », portée de son tableau de bord, sort des tâches d'un projet supprimé) et FR-28 (case « Hors projet » dans le filtre).
...
**Décision de conception — tâche "Sans projet" masquée dès qu'un filtre actif est appliqué.** ⚠️ **RÉVISÉE le 2026-09-21** (cf. Review Findings, entrée Post-livraison) : « Hors projet » est devenue une entrée de filtre cochable (`NO_PROJECT_FILTER_ID`), les tâches générales ne sont donc plus masquées inconditionnellement par une sélection de projets. Le raisonnement d'origine est conservé ci-dessous pour mémoire. Ni les FR ni les AC ne mentionnent explicitement le sort de la tâche générale (FR-2, `projectId: null`) quand un filtre de projet est actif. Lecture retenue : l'AC#1 dit littéralement "seules les tâches de CES projets restent affichées" — une tâche qui n'appartient à aucun projet n'est jamais une tâche "de ces projets", elle est donc exclue dès que `selectedProjectIds` n'est pas vide. Sans filtre actif (état par défaut), elle reste visible comme avant cette story (comportement inchangé de la Story 4.1). Si Guillaume juge ce comportement contre-intuitif en usage réel, ajuster `filterTasksForCalendar` (aucune AC ne fixe ce cas précisément) — documenter le changement s'il a lieu.
